  ...
  batch: '',
  status: '',
  has_sponsorship: '', // Keep this name for consistency
  role: ''
});
  ...
  const uniqueSponsor = useMemo(() => {
    const sponsoredStudents = students.filter(s => s.has_sponsorship); // Only those with sponsor
    return getUniqueValues(sponsoredStudents, 'sponsor_name'); // Make sure you're using the correct key
  }, [students]);
...
// Updated filteredStudents logic with sponsor filtering
const filteredStudents = useMemo(() => {
  return students.filter(student => {
    ...
    // NEW: Add sponsor filtering logic
    const matchesSponsor = !filters.has_sponsorship || (() => {
      if (filters.has_sponsorship === 'has') {
        return student.has_sponsorship === true;
      }
      if (filters.has_sponsorship === 'none') {
        return !student.has_sponsorship;
      }
      // If a specific sponsor name is selected
      if (student.has_sponsorship && student.sponsor_name) {
        return student.sponsor_name.toLowerCase().includes(filters.has_sponsorship.toLowerCase());
      }
      return false;
    })();

    return matchesName && matchesCourse && matchesBatch && matchesStatus && matchesSponsor;
  });
          ...
          <div style={styles.filterGrid}>
            ...
            {activeTab === 'students' && (
              <>
                ...
                <div style={styles.filterGroup}>
                  <label style={styles.filterLabel}>Sponsors</label>
                  <select
                    value={filters.has_sponsorship} // Changed from sponsor_info to has_sponsorship
                    onChange={(e) => handleFilterChange('has_sponsorship', e.target.value)}
                    style={styles.filterSelect}
                  >
                    <option value="">All Students</option>
                    <option value="has">Has Sponsor</option>
                    <option value="none">Without Sponsor</option>
                    {uniqueSponsor.map(sponsor => (
